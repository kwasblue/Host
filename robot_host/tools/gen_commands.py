...
def generate_json(commands: dict) -> str:
    """
    Emit commands.json in the format:
    ...
    """
    data = {
        "commands": commands,        # your COMMANDS dict
        "schema_version": SCHEMA_VERSION,
    }
    # indent=2 for the pretty structure you showed
    # no sort_keys so key order stays: commands, schema_version
...
def python_type_for(field: dict[str, Any]) -> str:
    """Map your schema type -> Python type string for hints."""
...
def write_python_client_mixin(commands: dict[str, dict]) -> str:
    """
    Generate Python mixin source as a string.
    main() will handle writing it to PY_CLIENT_MIXIN_OUT.
    """
    lines: list[str] = []
    ...
    for cmd_name, spec in commands.items():
        ...
        optional_args: list[tuple[str, str, Any, bool]] = []  # (name, type, default, has_default)
...
def main():
    ...
    #PY_CLIENT_MIXIN_OUT.parent.mkdir(parents=True, exist_ok=True)
